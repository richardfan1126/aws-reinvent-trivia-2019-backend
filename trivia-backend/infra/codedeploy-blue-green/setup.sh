...
aws cloudformation deploy --region us-east-1 --template-file packaged-template.yaml --stack-name TriviaBackendHooksTest --capabilities CAPABILITY_IAM --parameter-overrides TriviaBackendDomain=demo-api-test.hsing-hui-demo.richardfan.xyz

aws cloudformation deploy --region us-east-1 --template-file packaged-template.yaml --stack-name TriviaBackendHooksProd --capabilities CAPABILITY_IAM --parameter-overrides TriviaBackendDomain=demo-api.hsing-hui-demo.richardfan.xyz
...
sed -i.bak "s|<PLACEHOLDER>|$ACCOUNT_ID.dkr.ecr.us-east-1.amazonaws.com/reinvent-trivia-backend:latest|g" build/task-definition-test.json build/task-definition-prod.json
...
aws ecs create-service --region us-east-1 --service-name trivia-demo-backend-prod --cli-input-json file://build/service-definition-prod.json
